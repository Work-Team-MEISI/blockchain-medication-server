...
export class RecipeController {
    ...
    public async fetchRecipes(): Promise<Array<FetchResponseModel<RecipeModel>>> {
        ...
        for (const recipe of recipes) {
            responses.push(new FetchResponseModel<RecipeModel>(
                recipe.recipeId,
                recipe
            ));
        }
    ...
    @UseGuards(AccessTokenGuard)
    @Get("/:id")
    public async fetchRecipesByCitizenId(@Param("id") citizenId: FetchRecipeByCitizenDTO): Promise<Array<FetchResponseModel<RecipeModel>>> {
        const recipes = await this._recipeService.fetchBulkByHttpParam(citizenId).catch((error) => {
            throw new InternalServerErrorException(error);
        });
    ...
    @UseGuards(AccessTokenGuard)
    @Post()
    public async createRecipe(@Body() createRecipeDTO: CreateRecipeDTO): Promise<Array<CreateResponseModel<RecipeModel>>> {
        const medications: Array<Omit<MedicationModel, "basePrice" | "createdAt" | "updatedAt">> = new Array<Omit<MedicationModel, "basePrice" & "createdAt" & "updatedAt">>();

        for(const medicationId of createRecipeDTO.medications) {
            const med = await this._medicationService.fetchOne({medicationId: medicationId}).catch((error) => {
                throw new InternalServerErrorException(error);
            });
            
            const medication: Omit<MedicationModel, "basePrice" | "createdAt" | "updatedAt"> = {
        ...
        const recipes = await this._recipeService.fetchBulkByHttpParam(createRecipeDTO.citizenId).catch((error) => {
            throw new InternalServerErrorException(error);
        });
        ...
        for (const recipe of recipes) {
            responses.push(new CreateResponseModel<RecipeModel>(
                recipe.recipeId,
                null,
                recipe
            ));
